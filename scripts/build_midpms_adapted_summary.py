...
MIDPMS_DIR = RESULTS_ROOT / "paper_baselines" / "midpms_adapted"
SUMMARY_DIR = RESULTS_ROOT / "summary"

MODEL_NAMES = {
    "midpms_macro": "MIDPMS-macro",
    "midpms_micro": "MIDPMS-micro",
    "midpms_adapted": "MIDPMS-adapted",
}
...
def load_long_rows() -> list[dict[str, Any]]:
    rows = []
    for path in sorted(MIDPMS_DIR.glob("*_metrics.json")):
...
def main() -> None:
    ...
    if not long_rows:
        raise FileNotFoundError(f"No MIDPMS-adapted metrics found under {MIDPMS_DIR}")
    ...
    write_csv(SUMMARY_DIR / "paper_midpms_adapted_all_runs.csv", long_rows, long_fields)
    for basename, rows, fields in [
        ("paper_midpms_adapted_table", paper_rows(long_rows), paper_fields),
        ("paper_midpms_adapted_best", best_rows(long_rows), best_fields),
    ]:
    ...
    print(
        json.dumps(
            {
                "long": str(SUMMARY_DIR / "paper_midpms_adapted_all_runs.csv"),
                "paper_table": str(SUMMARY_DIR / "paper_midpms_adapted_table.csv"),
                "best_table": str(SUMMARY_DIR / "paper_midpms_adapted_best.csv"),
            },
            ensure_ascii=False,
            indent=2,
        )
