...
DYNAMICS_DIR = RESULTS_ROOT / "paper_baselines" / "dynamics"
...
def load_long_rows() -> list[dict[str, Any]]:
    rows = []
    for path in sorted(DYNAMICS_DIR.glob("*_metrics.json")):
                ...
                rows.append(
                    {
                        ...
                        "mean_final_size": metrics.get("mean_final_size"),
                        "mean_observed_size": metrics.get("mean_observed_size"),
                        "mean_observed_ratio": metrics.get("mean_observed_ratio"),
...
def paper_rows(long_rows: list[dict[str, Any]], split_strategy: str = "stratified") -> list[dict[str, Any]]:
    test_rows = [
        ...
        if row["split"] == "test" and row["split_strategy"] == split_strategy
    ]
    grouped: dict[tuple[str, int], list[dict[str, Any]]] = {}
    for row in test_rows:
        key = (row["dataset"], int(float(row["window_minutes"])))
...
def main() -> None:
    ...
    if not long_rows:
        raise FileNotFoundError(f"No dynamics metrics found under {DYNAMICS_DIR}")
    ...
    write_csv(SUMMARY_DIR / "paper_dynamics_baseline_all_runs.csv", long_rows, long_fields)

    paper = paper_rows(long_rows, split_strategy="stratified")
    for basename, rows, fields in [
        ("paper_dynamics_baseline_table", paper, paper_fields),
        ("paper_dynamics_baseline_best", best_rows(long_rows), best_fields),
    ]:
    ...
    print(
        json.dumps(
            {
                "long": str(SUMMARY_DIR / "paper_dynamics_baseline_all_runs.csv"),
                "paper_table": str(SUMMARY_DIR / "paper_dynamics_baseline_table.csv"),
                "best_table": str(SUMMARY_DIR / "paper_dynamics_baseline_best.csv"),
            },
            ensure_ascii=False,
            indent=2,
        )
